...
import { KashmiriWhatsAppStudio } from './KashmiriWhatsAppStudio';
...
export const LeadDetailModal: React.FC<LeadDetailModalProps> = ({
  ...
  const pitch = lead.pitches?.[0]?.message || generateKashmiriWhatsAppTemplate(lead);
  const whatsappUrl = buildWhatsAppTriggerUrl(lead.phone, pitch).universalUrl;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl max-w-2xl w-full max-h-[92vh] overflow-y-auto border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col">
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-3 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-10">
          <div className="min-w-0 flex-1">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white truncate">{lead.name}</h2>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all cursor-pointer ${
                isPitched
            ...
            >
              ...
              />
              <span className="hidden xs:inline">{isPitched ? 'Pitched ★' : 'Star'}</span>
            </button>
        ...
        <div className="p-4 sm:p-6 space-y-5 sm:space-y-6">
          ...
          {/* Personalized Kashmiri WhatsApp Outreach Studio */}
          <KashmiriWhatsAppStudio
            lead={lead}
            onPitchTriggered={(updatedLead) => {
              onUpdateLead?.(lead.id, {
                hasBeenPitched: true,
                isStarred: true,
                pitchedAt: updatedLead.pitchedAt,
                status: updatedLead.status,
                pitches: updatedLead.pitches,
              });
              if (updatedLead.status && updatedLead.status !== lead.status) {
                onStatusChange(lead.id, updatedLead.status);
              }
            }}
          />
        </div>
